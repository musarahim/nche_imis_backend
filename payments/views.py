...
class TINDetailsView(APIView):
    
    permission_classes = [permissions.AllowAny]
    def post(self, request):
        tin = request.data.get('tin')
        if not tin:
            return Response({"error": "tin is required."}, status=status.HTTP_400_BAD_REQUEST)
        svc = UraMdaPaymentService()
        try:
            data = svc.get_client_registration(tin)
            return Response(data, status=status.HTTP_200_OK)
        except Exception as ex:
            print(f"Error retrieving TIN details for {tin}: {str(ex)}")
            return Response({"error": str(ex)}, status=status.HTTP_400_BAD_REQUEST)